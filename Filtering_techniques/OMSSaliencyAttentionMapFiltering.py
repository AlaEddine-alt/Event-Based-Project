...
class OMSAttentionFiltering:
    ...
    def OMS_filtering(self):
        ...
        # --- Apply Attention on OMS map ---
        S = OMS_map.astype(np.float32)
        att_in = np.stack([self.window_pos, self.window_neg], axis=0)  # shape [2, H, W]
        att_in = torch.from_numpy(att_in).float().to(self.config.DEVICE)
        att_in = att_in.unsqueeze(0)  # shape [1, 2, H, W]
