...
#from functions.mv_helpers import *


class Config:
    RESOLUTION = [640, 480]  # Resolution of the DVS sensor
    MAX_X = RESOLUTION[0]
    MAX_Y = RESOLUTION[1]
    BATCH_SIZE = 0000
...
def make_image(events, start_time, max_time, duration):
    if start_time +duration > max_time:
        print("reached end of event")
        return None
    
    i = start_time
    image = np.zeros((config.RESOLUTION[1], config.RESOLUTION[0]))
    print(start_time, )
    while int(events[i][-2]) < start_time+duration:
        x, y, t, p = [int(i) for i in events[i]]
        image[y, x] +=p 
        i+=1
...
if __name__ == '__main__':
    ...
    events = [] #pd.read_csv('events.txt', sep=" ",header = None)
    ...
    # Initialize OMS and Attention Module
    ...
    # Initialize empty event buffers
    #numevs = [len(events)]  # Total number of events
    window_pos = np.zeros((config.RESOLUTION[1], config.RESOLUTION[0]))  # Positive events
    window_neg = np.zeros((config.RESOLUTION[1], config.RESOLUTION[0]))  # Negative events
    ...
    """for i in range(1, len(events), config.BATCH_SIZE):  # Simulate live processing
        current_time = time.time()
        print(current_time - start_time)
        if current_time - start_time > 20:
            print(f"Mean of events: {np.round(np.mean(events_count), 2)}")
            print(f"Mean of suppressed events: {np.round(np.mean(suppressed), 2)}")
            break
        
        batch = events[i:i + config.BATCH_SIZE]  # Read a batch of events
        #print(batch"""
    #height, width, events = get_events()
    ...
    with torch.no_grad():
        ...
        print(int(events[-1][-2]))
        for  i in range(1, int(events[-1][-2]), 5000):
            #print(batch)
            current_time = time.time()
            print(current_time - start_time)
            
            window_pos = make_image(events, i, int(events[-1][-2]), 5000)

            with buffer_lock:
                numevs = [np.sum(window_pos)]
                if current_time - last_update_time > config.UPDATE_INTERVAL:
                    print("here "+ str(last_update_time))
                    if numevs[0] > 0:
                        OMS, indexes = compute_OMS(window_pos)
                        events_count.append(numevs[0])
                        suppressed.append(indexes.sum().item())
                        dropped.append(numevs[0] - indexes.sum().item())

                        print(events_count, dropped, suppressed)

                        graph_img = draw_graph_with_dots(events_count, suppressed, dropped)
                        ...
                        window_pos_resized = convert_to_rgb(cv2.resize(np.flipud(window_pos.astype(np.float32)), (scaled_width, scaled_height)))
