...
def net_def(filter, tau_mem, in_ch, out_ch, size_krn, device, stride):
    ...
    net = nn.Sequential(
        nn.Conv2d(in_ch, out_ch, (size_krn,size_krn), padding=3, stride=stride, bias=False),
        sl.LIF(tau_mem),
    )
...
def egomotion(window, net_center, net_surround, device, max_y, max_x, threshold):
    ...
    events = 1 - (events - events.min()) / (events.max() - events.min() + 1e-6)
    
    OMS = events * 255  # scale to display range
    
    # Apply threshold to obtain binary/filtered saliency map
    OMS_thresholded = torch.zeros_like(OMS)
    indexes = events >= threshold
    OMS_thresholded[indexes] = OMS[indexes]

    # Free unused GPU memory
    torch.cuda.empty_cache()

    return OMS_thresholded, indexes


def egomotionRomaric(window, net_center, net_surround, device, max_y, max_x,threshold):
    window = window.unsqueeze(0).float().to(device)
    center = net_center(window)
    surround = net_surround(window)
    events = center - surround
...
def initialize_oms(device,OMS_PARAMS):
    ...
    if torch.cuda.device_count() > 1:
        print(f'yeah: {torch.cuda.device_count()}GPUs')
        net_center= nn.DataParallel(net_center)
        net_surround=nn.DataParallel(net_surround)
